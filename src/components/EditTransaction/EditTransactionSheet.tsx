...
function EditTransactionSheet(props: {
  variant: string;
  api: string;
  ...
  pagingData: (
    dataPaged: { id: number; date: string; billName: string; amount: number }[]
  ) => void;
}) {
  ...
  const [categoryIdPlaceHolder, setCategoryIdPlaceHolder] =
    useState<string>('');
  ...
  useEffect(() => {
    axios
      .get(`${props.api}/single?id=${props.transactionId}`)
      .then((result) => {
  ...
  const update = (variant: string, value: string | number) => {
    axios
      .patch(
        `${props.api}/${variant}?id=${props.transactionId}&${variant}=${value}`
      )
      .then((response) => {
        if (response.status === 200) {
          axios
            .get(`${props.api}?pageNumber=${props.pageNumber}`)
            .then((response) => {
    ...
    <Sheet>
      ...
      <SheetContent className='bg-slate-900 text-slate-50'>
        <SheetHeader>
          ...
          <SheetDescription>
            Edit transaction by completing the form below. You can only edit one
            field at a time. Check the checkbox next to the field you want to
            edit
          </SheetDescription>
        </SheetHeader>
        <div className='grid gap-4 py-4'>
          <FormComponent
            ...
            variant={'name'}
            getDataFromFormComponent={getDataFromFormComponent}
            toggleDisabled={toggleDisabled}
            checked={isCheckedName}
          />
          <div className='grid grid-cols-5 gap-4 items-center'>
            <Label htmlFor='select' className='text-right capitalize'>
              Status
            </Label>
            ...
            <Checkbox
              className='bg-slate-50 col-span-1'
              checked={isCheckedStatus}
              onCheckedChange={() =>
                toggleDisabled(disabledStatus, setDisabledStatus, 'status')
              }
            />
          </div>
          <div className='grid grid-cols-5 gap-4 items-center'>
            <Label htmlFor='bill' className='text-right capitalize'>
              Bill
            </Label>
            <SelectData
              variant='bill'
              getDataFromSelectData={getDataFromSelectData}
              disabled={disabledBill}
              placeHolder={billIdPlaceHolder}
            />
            <Checkbox
              className='bg-slate-50 col-span-1'
              checked={isCheckedBill}
              onCheckedChange={() =>
                toggleDisabled(disabledBill, setDisabledBill, 'bill')
              }
            />
          </div>
          <FormComponent
            ...
            setDisabled={setDisabledAmount}
            placeHolder={amountPlaceHolder.toFixed(2).toString()}
            variant={'amount'}
            getDataFromFormComponent={getDataFromFormComponent}
            toggleDisabled={toggleDisabled}
